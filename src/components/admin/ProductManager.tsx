...
export function ProductManager() {
  ...
  const edit = (row: Row) => {
    ...
    const sizes =
      product.options?.find((option: any) => option.name?.toLowerCase() === "size")?.values ?? [];
    setDraft({
  ...
  const uploadImages = async (files: FileList | null) => {
    ...
    try {
      ...
      setDraft((current) =>
        current ? { ...current, images: [...current.images, ...urls] } : current,
      );
    } catch (error) {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (!Number.isFinite(priceUsd) || priceUsd <= 0)
      return setMessage("Enter a valid USD base price.");
    setBusy(true);
    setMessage(null);
    try {
      await saveProduct({
        data: {
          ...
          images: draft.images.filter(
            (image) => /^https?:\/\//.test(image) || image.startsWith("/api/public/backup-image"),
          ),
          sizes: draft.sizes
            .split(",")
            .map((size) => size.trim())
  ...
  return (
    <section className="mt-10 border-y border-border py-8">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            USD is your base price. Customers see their local display currency; checkout charges the
            GHS amount.
          </p>
        </div>
        ...
        <table className="w-full min-w-[760px] text-sm">
          <thead className="bg-muted text-[10px] tracking-brand-wide uppercase text-muted-foreground">
            <tr>
              ...
              <th className="px-4 py-3 text-right">Displayed in Ghana</th>
              <th className="px-4 py-3 text-right">Checkout GHS</th>
              ...
              return (
                <tr key={row.handle} className="border-t border-border">
                  ...
                  <td className="px-4 py-3 text-right tabular-nums">
                    GH₵{(usd * GHS.rate).toFixed(2)}
                  </td>
                  <td className="px-4 py-3">
                    <span
                      className={row.is_published ? "text-emerald-600" : "text-muted-foreground"}
                    >
      ...
      {draft && (
        <form
          onSubmit={submit}
          className="mt-6 grid gap-5 border border-border bg-card p-5 md:grid-cols-2"
        >
          <div>
            <label className="text-[10px] tracking-brand-wide uppercase text-muted-foreground">
              Product name
            </label>
            <Input
              className="mt-1"
              ...
              onChange={(e) =>
                setDraft({
                  ...
                  handle: draft.originalHandle ? draft.handle : slugify(e.target.value),
                })
              }
            />
          </div>
          <div>
            ...
            <Input
              ...
              required
              value={draft.handle}
              onChange={(e) => setDraft({ ...draft, handle: slugify(e.target.value) })}
            />
          </div>
          <div>
            <label className="text-[10px] tracking-brand-wide uppercase text-muted-foreground">
            ...
            <p className="mt-2 text-xs text-muted-foreground">
              Customer display: {formatLocal(pricePreview, GHS)} in Ghana · Checkout: GH₵
              {(pricePreview * GHS.rate).toFixed(2)}
            </p>
          </div>
          <div>
            <label className="text-[10px] tracking-brand-wide uppercase text-muted-foreground">
          ...
          <div className="md:col-span-2">
            ...
            <textarea
              className="mt-1 min-h-28 w-full rounded-md border border-input bg-transparent px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
              maxLength={5000}
          ...
          <div className="md:col-span-2">
            ...
            <Input
              ...
              disabled={uploading || draft.images.length >= 12}
            />
            <div className="mt-3 flex flex-wrap gap-2">
              {draft.images.map((image, index) => (
                <button
                  ...
                  className="group relative h-20 w-16 overflow-hidden border border-border"
                >
                  <img src={image} alt="" className="h-full w-full object-cover" />
                  <span className="absolute inset-x-0 bottom-0 bg-background/90 py-1 text-[9px] uppercase opacity-0 group-hover:opacity-100">
                    Remove
                  </span>
                </button>
              ))}
